...
from scraping_service.settings import EMAIL_HOST_USER, EMAIL_HOST_PASSWORD, EMAIL_HOST
...
User = get_user_model()
# получить всех активных пользователей-подписчиков
qs = User.objects.filter(send_email=True).values('city', 'language', 'email')
users_dct = {}
for i in qs:
    # ключ=город+language, value=email
...
if users_dct:
    ...
    qs = Vacancy.objects.filter(**params, timestamp=today).values()[:5]
    vacancies = {}
    for i in qs:
        vacancies.setdefault((i['city_id'], i['language_id']), [])
        vacancies[(i['city_id'], i['language_id'])].append(i)
    # формирование html-содержимого писем
